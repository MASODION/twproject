      ...
      elseif(isset($_GET['action']) && $_GET['action'] == "aobiecte") {
        if(isset($_POST['obiect']) && isset($_POST['cantitate'])) {
          ...
          if($cantitate > 0) {
            ...
            $countlog = $check->rowCount();
            if($countlog > 0) {
              $check = $conn->prepare("SELECT COUNT(oiCantitate) as cantitate FROM obiecte_interventie WHERE oiInterventieID = '$id' AND oiObiectID = '$id_obiect'");
              $check->execute();
              $countlog = $check->rowCount();
              if($countlog > 0) {
                $row = $check->fetch();
          ...
          <div class="row">
            <div class="col-6">
              <div class="card">
                ...
                <p style="margin-top:-25px;"><label>Pret: <?=$programarea['cPret']?> lei</label></p>
